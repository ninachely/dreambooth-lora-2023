...
В оригинальной статье при проведении экспериментов на основе Stable Diffusion было выставлено значение ```LEARNING_RATE=5e-6```, поэтому мне показалось разумным начать с него. Однако результаты были неудовлетворительны, поэтому я стала немного увеличивать значение

- ```LEARNING_RATE_TEXT_ENCODER=1e-5```

Я попробовала выставить ```1e-4```, получилось плохо

Затем попробовала выставить ```1e-6```, тоже получилось плохо
...
![Несколько иозбражений из датасета ```rc_car```](/images/dataset.png "Рис. 1")
